...
class page1 extends JFrame implements MouseListener,MouseMotionListener ,ActionListener
{
	...
	page1()
	{
	   ...
	   //cb1a.addActionListener(this);
		...
		fs=new JButton("Find No of segment");
	...
	public void actionPerformed(ActionEvent ae)
	{
	   ...
	   if(ae.getSource()==cb)
	   {
		...
		if(ae.getSource()==fs)
		{
			...
			int[] labelled=new int[(width*height)+1];
			int[] queue=new int[(width*height)+1];
			double temp,temp1;
			
			
			if(rgb==0)
			{
				JOptionPane.showMessageDialog((Component)null,"Please Click on Feature to Find Number of Segments","NOTIFICATION",JOptionPane.ERROR_MESSAGE);
			}
			else
			{
				...
				for(i=1;i<=height;i++)
				{
					for(j=1;j<=width;j++)
					{
						if(imageArray[i][j]==1 && visited[(i-1)*width+j]==0 && labelled[(i-1)*width+j]==0)
						{
							rear++;
							front++;
							queue[rear]=(i-1)*width+j;
							visited[(i-1)*width+j]=1;
							current_label++;
							labelled[(i-1)*width+j]=current_label;
				
							//now insert adjacent pixel one by one
							while(front<=rear)
							{
								temp=queue[front];
								temp1=width;
								row=(int)Math.ceil(temp/temp1);
								col=queue[front]%width;
								if(col==0)
									col=width;
								...
								for(k=-1;k<=1;k++)
								{
									for(l=-1;l<=1;l++)
									{
										if(imageArray[row+k][col+l]==1 && visited[(row+k-1)*width+col+l]==0 && labelled[(row+k-1)*width+col+l]==0)
										{
											rear++;
											queue[rear]=(row+k-1)*width+col+l;
											visited[(row+k-1)*width+col+l]=1;
											labelled[(row+k-1)*width+col+l]=current_label;
										}
									}
								}
								front++;
								visited[(row-1)*width+col]=1;
								labelled[(row-1)*width+col]=current_label;
							}
						}			
					//initialize queue
				...
				System.out.println("Total segments="+(current_label-1));
				tf4.setText(Integer.toString(current_label-1));
			}
	   }
	}
}